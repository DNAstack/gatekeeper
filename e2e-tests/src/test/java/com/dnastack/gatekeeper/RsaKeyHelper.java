...
class RsaKeyHelper {
	...
	private static RSAPrivateKey parseSSHPrivateKey(String encKey) {
	...
	static RSAPrivateKey createPrivateKey(BigInteger n, BigInteger e) {
		try {
			return (RSAPrivateKey) KeyFactory.getInstance("RSA").generatePrivate(new RSAPrivateKeySpec(n, e));
		}
		catch (Exception ex) {
